...
## Household View analytical extensions (Phase 26)

Phase 26 extended the Household View with editorial context and asset-location analytics: an advisor-vs-SAA strategic comparison section (26.0) and a suite of analytical panels covering drift sort, tax-drag ranking, sleeve substitution mapping, and concentration analysis (26.1). The demo-vs-personal split established in Phase 25 continues to hold — no Phase 26 features are accessible in demo mode. The 22 macro render tests in `tests/render/test_macro_render.py` were marked `@pytest.mark.live_data` and excluded from the default suite via `addopts` in `pytest.ini`; they run only when opted in with `-m live_data`, keeping the default suite deterministic in CI.
