...
with col:
    ...
    if t10y2y is not None:
        ...
        with col_w:
            st.caption(
                f"Current: {sign}{current_spread_bps:.0f} bps  \n"
                f"Window pct: {_ordinal(yc_pctile_w)}"
            )
    ...
    if dff is not None:
        ...
        with col_w:
            st.caption(
                f"Current: {current_ff:.2f}%  \n"
                f"Window pct: {_ordinal(ff_pctile_w)}"
            )
    ...
    if hy_oas is not None:
        ...
        with col_w:
            st.caption(
                f"Current: {current_hy:.0f} bps  \n"
                f"Available pct: {_ordinal(hy_pctile_w)}"
            )
    ...
    if unrate is not None:
        ...
        with col_w:
            st.caption(
                f"Current: {current_ur:.1f}%  \n"
                f"Window pct: {_ordinal(ur_pctile_w)}"
            )
    ...
    if gdp_gr is not None:
        ...
        with col_w:
            st.caption(
                f"Current: {current_gdp:.1f}%  \n"
                f"Window pct: {_ordinal(gdp_pctile_w)}"
            )
    ...
    if core_cpi is not None:
        ...
        with col_w:
            st.caption(
                f"Current: {current_cpi:.1f}%  \n"
                f"Window pct: {_ordinal(cpi_pctile_w)}"
            )
    ...
    try:
        ...
        with col_w:
            st.caption(
                f"Window pct: {_ordinal(ratio_pctile_w)}"
            )
