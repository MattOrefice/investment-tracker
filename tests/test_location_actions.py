...
RENDERED_PROSE_LEN = {
    ...
    "frozen_tod_income":         (626, 523),   # pros +225 (#210): {annual_benefit} was resolving to a dollar figure; GAOSX is a multi_asset blend whose look-through hits real_assets_commodities, now NOT-MODELLED, so the blend refuses WHOLE (src/household.py) and the card states why the annual cost is withheld instead of citing one. NOTE this pin is now STATE-DEPENDENT: it captures the refusing branch of {drag_amount}. If commodities ever become sizable the sized branch renders and this length changes — which is the pin working, not drifting. Earlier, pros +1: GAOSX is now sized by looking through its fund_compositions holdings (2.66%) instead of taking the 1.80% equity default, so the group's {annual_benefit} went $99 -> $122 — one more digit. Earlier: literal "Three of them" -> derived {register_count} + JCPB joins the enumeration (Aug-2026 advisor swap)
    "saa_sleeves_taxable":       (405, 977),   # pros +71 / cons +329 (#210): PDBC is one of this group's four rows and is now NOT-MODELLED — a commodity fund distributes realized gains, so a TTM yield has no steady-state value (0.0%-40.4% across eight years). Both {annual_benefit} citations become stateful sentences: pros says the cost is not stated and why to look below, cons states that the drag is real and deliberately unsized, carries the range inline so a reader can check it, and says the other three holdings ARE sized — the refusal is narrow. The cons note below is now HISTORICAL: it said cons was UNCHANGED at 648 because the sentence said "The drag is {annual_benefit} a year" unqualified; that sentence is exactly what #210 had to replace, since the figure it cited can no longer be computed. STATE-DEPENDENT like frozen_tod_income above. Earlier, pros: accrual attributed to the TIPS sleeve, not applied to all four holdings — only SCHP throws it; VGIT/VNQ/PDBC generate ordinary income, PDBC being the no-K-1 1099 wrapper. #213 turned "plus the phantom income" into "including the inflation accruals" (-6) once the constructed yield put the accrual INSIDE {annual_benefit}. Cons UNCHANGED at 648, which is the evidence rather than the assumption: it says "The drag is {annual_benefit} a year" unqualified, so it was incomplete at 0.025 and is correct now with no edit — and $3.40 -> $5.08 is the same width, so the pin proves the prose held while the figure moved. Cons also carries the capacity restatement -> cross-ref to clear_roth_non_equity
