...
SLEEVE_ASSUMED_YIELD: dict[str, float] = {
    ...
    "high_yield_fi":           0.060,
    "high_yield_muni":         0.040,   # real distribution yield; exemption is separate (see FEDERALLY_EXEMPT_SLEEVES)
    "floating_rate":           0.055,
...
# Sleeves whose income is EXEMPT FROM FEDERAL TAX (municipal bonds). Tax status is
# a SEPARATE fact from yield: the table above is what the sleeve throws off; this
# set is whether that income is federally taxable. build_location_register charges
# a federally-exempt sleeve the STATE rate only, never the combined federal+state
# ordinary rate — so its income-shelter value at stake is correctly small rather
# than a phantom federal saving. PA taxes out-of-state muni interest, and a
# national HY-muni fund (e.g. GHYIX, sleeve high_yield_muni) is mostly out-of-state,
# so PA's flat 3.07% still applies: the exemption is federal, not total. Keyed on
# the SLEEVE, never a symbol list. USER-EDITABLE.
